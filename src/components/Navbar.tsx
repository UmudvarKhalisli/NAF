...
export default function Navbar() {
  ...
  return (
    <>
      {/* Navbar */}
      <nav
        className={`fixed top-0 left-0 right-0 z-[100] transition-all duration-500 ${forceSolid
          ...
          <div className="flex items-center justify-between gap-4">
            ...
            <div className="hidden lg:flex items-center gap-8 xl:gap-10">
              {navLinks.map((link) => (
                <Link
                  key={link.label}
                  href={link.href}
                  className={`relative text-[15px] font-bold tracking-wide transition-all duration-300 group ${activeSection === link.id && isHome
                      ? (forceSolid ? "text-neutral-900" : "text-gray-300")
                      : (forceSolid ? "text-black/60 hover:text-black" : "text-white/70 hover:text-white")
                    }`}
                >
                  {link.label}
                  <span className={`absolute -bottom-1 left-0 h-[2px] bg-neutral-700 transition-all duration-300 ${activeSection === link.id && isHome ? "w-full" : "w-0 group-hover:w-full"
                    }`} />
                </Link>
              ))}
            </div>
            ...
            <div className="flex items-center gap-4 lg:gap-8 flex-shrink-0">
              ...
              <a
                ...
                className={`hidden md:block px-6 py-2.5 text-[11px] font-black tracking-[0.2em] uppercase border transition-all duration-500 rounded-lg ${forceSolid || open
          ...
          >
              ...
              {navLinks.map((link, index) => (
                ...
                >
                  <Link
                    ...
                    className={`text-xl font-black tracking-tight transition-colors block ${activeSection === link.id && isHome ? "text-neutral-400" : "text-black"
                      }`}
                  >
                    {link.label}
                  </Link>
            ...
            <div className="pt-6 pb-8 border-t border-black/5 space-y-5">
              ...
              <div className="flex gap-2">
                ...
                <a href="https://www.tiktok.com/@naftexnika.az?_r=1&_t=ZS-95VApVTzIfA" target="_blank" rel="noopener noreferrer" className="w-9 h-9 rounded-lg bg-black/5 flex items-center justify-center text-black/40 hover:bg-black/10 transition-colors">
